import gc
from machine import I2C, Pin, ADC, UART, deepsleep, reset
import esp32
...
lcd_mosfet = Pin(ON_LCD, Pin.OUT, value=1)
...
index = ''
with open('mat_index.dat', 'r') as f:
    index = f.read()
...
class Delver:
    ...
    def step_2(self):
        ...
        self.stop = False
        _thread.start_new_thread(self.weigth_monitor_thread, ())

    def weigth_monitor_thread(self):
        while not self.stop:
            p = self.load_cell.get_value()
            p_neto = int((p[0] * self.load_cell.get_factor()))
            self.lcd.move_to(4, 1)
            self.lcd.putstr("{} {}".format(p_neto, "E" if p[1] * self.load_cell.get_factor() < 2 else " "))
            self.lcd.move_to(12, 1)
...
esp32.wake_on_ext1(pins=(delver.btn3.pin,), level=esp32.WAKEUP_ANY_HIGH)
